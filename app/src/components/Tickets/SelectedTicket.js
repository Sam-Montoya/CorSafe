...
class SelectedTicket extends Component {
		...
		if (this.state.selectedTicket.ticket_id === 0) {
			...
			if (this.props.user.auth_id) {
				axiosController.postComment(this.state.postComment).then(() => {
					this.updateComments();
					axiosController
						.updateNotificationStatus(this.state.selectedTicket.ticket_id, true)
						.then((response) => {
	...
	updateNavButton(status) {
		...
		if (status === 'Resolved') this.props.updateButtonClass('top-navbar-button-resolved');
		else if (status === 'Not Answered') this.props.updateButtonClass('top-navbar-button-notanswered');
		else this.props.updateButtonClass('top-navbar-button-inprogress');
	}
							...
							<div className="submit-ticket_topcontainer">
								<section className="submit-ticket_subjectcontainer">
									...
									<input
										...
										type="text"
										value={this.state.selectedTicket.subject}
										disabled
									/>
								</section>
								...
								<section className="submit-ticket_tagcontainer">
									...
									<input
										style={{ backgroundColor: 'rgb(250, 250, 250)' }}
										type="text"
										value={this.state.selectedTicket.tag}
										disabled
									/>
								</section>
							</div>
								...
								{this.props.user.role === 'user' ? (
									<section className="cancelbutton_container">
										<Link to="/dashboard/my-tickets">
											<button className="submit-ticket_buttoncontainer_cancel">Back</button>
										</Link>
									</section>
								) : (
									<div className="selectedticket_buttons">
										<section className="adminbuttons_container">
											<button
												className="selectedticket_resolved"
												onClick={() => this.updateTicketStatus('Resolved')}>
												Resolved
											</button>
											<button
												className="selectedticket_inprogress"
												onClick={() => this.updateTicketStatus('In-Progress')}>
												In-Progress
											</button>
						...
						<div className="selected-ticket_comments">
							...
							<div className="add_comment_container">
								...
								<textarea
									...
									id="submitarea"
									onChange={(input) => this.updateCommentBox(input.target.value)}
									className="comment_commentconainer"
								/>
							</div>
							<section className="selectedticket-submitcontainer">
								<button className="selected-ticket_commentbutton" onClick={() => this.postComment()}>
									Comment
								</button>
							</section>
						</div>
